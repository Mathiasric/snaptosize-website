"""Schedule W23 Day-8 Pinterest pins (2026-05-19) via Buffer API.
G10: gallery-wall-sizes   — The 5 sizes that make every gallery wall work. (Gemini)
G11: listing-gap          — What you list vs what buyers can actually frame. (Gemini)
P16: resolution-guide     — Is your image high-res enough to print? (React)
"""
...
PINS = [
    {
        ...
        "due_at":  "2026-05-20T13:00:00Z",  # 3pm GMT+2
    },
    {
        "item_id": "W23-G11",
        "slug":    "2026-05-19-listing-gap-pinterest",
        "board":   "digital-prints",
        "due_at":  "2026-05-20T16:00:00Z",  # 6pm GMT+2
    },
    {
        "item_id": "W23-P16",
        "slug":    "2026-05-19-resolution-guide-pinterest",
        "board":   "digital-prints",
        "due_at":  "2026-05-20T19:00:00Z",  # 9pm GMT+2
    },
]

dry_run = "--dry-run" in sys.argv

print(f"{'DRY RUN -- ' if dry_run else ''}Scheduling {len(PINS)} W23 Day-8 pins...\n")
...
for pin in PINS:
    ...
    if dry_run:
        print(f"  DRY RUN -- would schedule to Buffer")
